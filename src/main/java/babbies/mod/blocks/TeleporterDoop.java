...
public class TeleporterDoop extends Teleporter {
	...
	public TeleporterDoop(WorldServer worldIn) {
		super(worldIn);
			...
			for (int l = -2; l <= 2; ++l) {
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag
								? ModBlocks.doopblock.getDefaultState()
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_) {
			...
			for (int l = -128; l <= 128; ++l) {
				...
				for (int i1 = -128; i1 <= 128; ++i1) {
					for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1) {
						...
						if (this.worldServerInstance.getBlockState(blockpos).getBlock() == ModBlocks.portalDoop) {
							while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.down()).getBlock() == ModBlocks.portalDoop) {
								blockpos = blockpos1;
							}
		...
		if (d0 >= 0.0D) {
			if (flag1) {
				this.destinationCoordinateCache.add(k, new TeleporterDoop.PortalPosition((BlockPos) object, this.worldServerInstance.getTotalWorldTime()));
			...
			if (this.worldServerInstance.getBlockState(((BlockPos) object).west()).getBlock() == ModBlocks.portalDoop) {
				enumfacing = EnumFacing.NORTH;
			}

			if (this.worldServerInstance.getBlockState(((BlockPos) object).east()).getBlock() == ModBlocks.portalDoop) {
				enumfacing = EnumFacing.SOUTH;
			}

			if (this.worldServerInstance.getBlockState(((BlockPos) object).north()).getBlock() == ModBlocks.portalDoop) {
				enumfacing = EnumFacing.EAST;
			}

			if (this.worldServerInstance.getBlockState(((BlockPos) object).south()).getBlock() == ModBlocks.portalDoop) {
	...
	public boolean makePortal(Entity p_85188_1_) {
		...
		IBlockState iblockstate = ModBlocks.portalDoop.getDefaultState().withProperty(BlockPortalDoop.AXIS, l5 != 0
			...
			for (k3 = 0; k3 < 4; ++k3) {
				for (l3 = -1; l3 < 4; ++l3) {
					...
					this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1
							? ModBlocks.doopblock.getDefaultState() : iblockstate, 2);
				}
			}
